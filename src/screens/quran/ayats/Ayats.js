...
export default function Ayats() {
  ...
  useEffect(() => {
    ...
    async function fetchdata(){
     try {
        await GetData(`http://api.alquran.cloud/v1/surah/${surahNumber}/ar.alafasy`).then((response)=>{
 ...
 const Loaddata = ()=>{
  ...
  const DisplaySurahs = ({item})=>{
    ...
    <Text style={styles.AyatNameUrdu}>{item.text}</Text>
      ...
      <Text style={styles.Surah}>{'Allah'}</Text>
    </View>
    )
  }
    ...
    <View style={styles.Container}>
      ...
      <View style={styles.Header}>
        <TouchableOpacity style={styles.IconDiv} onPress={()=>navigation.goBack()}>
          ...
          <TouchableOpacity style={styles.IconDiv} onPress={()=>{
          dispatch(AddSruahFav(data.name))
          }}>
          <Image source={require('../../../assets/icons/heart.png')} style={styles.Headericon}/>
        </TouchableOpacity>
      </View>
